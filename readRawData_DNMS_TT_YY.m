function [ rawData,timePointMat,timeMat,trialNo,blockNo,delayDuration, dailyPerf,typePerfPerBlock,perfPerBlock,trialMat,typeDailyPerf] = readRawData_DNMS_TT_YY( rawData,trialNoPerBlock)
...
% modified by Shimin on 8th May 2018, in this version, only TT or YY
    ...
    timePointMat = zeros(trialNo,6);
        ...
        timePointMat(:,1) = temp1(1:trialNo,:);
        temp2 = rawData (rawData(:,3)==20&rawData(:,4)==2,1);
        timePointMat(:,2) = temp2(1:trialNo,:);
        temp3 = rawData (rawData(:,3)==20&rawData(:,4)==0,1);
        timePointMat(:,3) = temp3(1:trialNo,:);
       %% choice phase
        ...
        timePointMat(:,6) = temp6(1:trialNo,1);
    ...
    for blockIndex = 1:ceil(blockNo)
        ...
        perfPerBlock(blockIndex,1) = 100*length(find(blockMat(:,3)==7))/length(blockMat(:,1));
        % type
        type1Block = blockMat(blockMat(:,1)<3,:);%TT:1-4, YY:2-3
        type2Block = blockMat(blockMat(:,1)>2,:);%TT:4-1, YY:3-2
        typePerfPerBlock(blockIndex,1) = 100*length(find(type1Block(:,3)==7))/length(type1Block(:,1));
        typePerfPerBlock(blockIndex,2) = 100*length(find(type2Block(:,3)==7))/length(type2Block(:,1));
    end 
